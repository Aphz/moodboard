...
| Sujetos recortados (stickers de iOS) | ✅ | Un PNG con transparencia —el sujeto que recorta iOS al crear un sticker, copiado en Fotos y pegado aquí— se comporta como lo que aparenta: el toque atraviesa su fondo vacío hasta lo que haya debajo (`src/features/alphaMask.ts`) y no se le dibuja el borde rectangular que delataría el recorte. La sombra ya salía con la forma del sujeto |
| Tocar lo que hay bajo un ornamento | ✅ | Una nota de fondo transparente —los ornamentos lo son— se toca sobre sus letras, no en todo su rectángulo: en un símbolo suelto eso devuelve más de la mitad de la caja a lo que tenga debajo (`src/features/noteText.ts`). Las notas con fondo no cambian |
