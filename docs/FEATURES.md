...
- **Clave API a prueba de descuidos.** Se guarda en IndexedDB con respaldo en `localStorage` y se restaura sola si Safari vacía la base; sólo se acepta una cadena con forma de clave, así que un campo vacío o una contraseña autocompletada no la borran, guardar cualquier otra preferencia nunca escribe una clave vacía encima de la buena, y quitarla es un botón con confirmación. Se puede copiar al portapapeles para guardarla en el llavero.
