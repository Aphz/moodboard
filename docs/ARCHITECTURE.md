...
- **Qué se toca** (`features/strokes.ts`, puro): las imágenes y las notas se resuelven con su caja, pero un **dibujo no**. Su caja envuelve los trazos y la tinta ocupa una parte mínima de ella, así que —como los dibujos se pintan siempre por encima de las imágenes— cualquier anotación dejaba inseleccionable la imagen que tenía debajo. `strokesHit` mide la distancia real a cada trazo (polilínea para el lápiz, segmento para línea y flecha, y el contorno hueco del rectángulo y la elipse), con una holgura de 8 px de pantalla convertida a unidades locales (`STROKE_SLOP / (zoom · scale)`) para que el dedo no tenga que acertar al píxel. Lo mismo con una **nota de fondo transparente** —y todo ornamento lo es—: su caja incluye el relleno y lo que sobra a los lados de las líneas cortas, así que la zona sensible se arma con la caja de las letras (`features/noteText.ts`, a partir de la maqueta que el renderizador ya cachea para dibujar; `renderer.noteTextBox`). Una nota **con** fondo tiene cuerpo y sigue midiéndose por su caja. Y una **imagen con transparencia** —un sujeto recortado— se mide contra su máscara de opacidad (`features/alphaMask.ts`): una rejilla de 48×48 celdas con el alfa de cada una, calculada la primera vez que hace falta y guardada junto al bitmap en `imageCache`. Lo grueso de la rejilla juega a favor: da holgura al dedo sin cálculos extra y ocupa 2 KB en vez de los megas del bitmap. La misma máscara le dice al renderizador que no dibuje el borde rectangular sobre un recorte. Excepción: un **dibujo o una nota** ya seleccionados vuelven a medirse por su caja, para poder arrastrarlos agarrándolos de cualquier parte; una imagen no entra en esa excepción, porque su sujeto ya es un blanco grande y un recorte con mucho vacío bloquearía media pantalla por estar seleccionado.
