...
- **Qué se toca** (`features/strokes.ts`, puro): las imágenes y las notas se resuelven con su caja, pero un **dibujo no**. Su caja envuelve los trazos y la tinta ocupa una parte mínima de ella, así que —como los dibujos se pintan siempre por encima de las imágenes— cualquier anotación dejaba inseleccionable la imagen que tenía debajo. `strokesHit` mide la distancia real a cada trazo (polilínea para el lápiz, segmento para línea y flecha, y el contorno hueco del rectángulo y la elipse), con una holgura de 8 px de pantalla convertida a unidades locales (`STROKE_SLOP / (zoom · scale)`) para que el dedo no tenga que acertar al píxel. Excepción: un dibujo **ya seleccionado** vuelve a medirse por su caja, para poder arrastrarlo agarrándolo de cualquier parte y no sólo de la tinta.
