...
export interface ICloudSyncPlugin {
   ...
   */
  list(options: { dir: string }): Promise<{ entries: ICloudEntry[] }>;

  /** Comprueba si existe una ruta y si su contenido ya está descargado. */
  exists(options: { path: string }): Promise<{ exists: boolean; downloaded: boolean }>;
  ...
  addListener(
    eventName: 'changed',
    listenerFunc: (event: { paths: string[] }) => void,
